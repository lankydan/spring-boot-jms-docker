...
```
Adding `-d` will run the container in the background. The name of the container is not just for convenience as it will be needed by the Spring application to connect to Mongo.
...
```
The names of the containers are prepended with the name of the project and the network follows the same logic.
